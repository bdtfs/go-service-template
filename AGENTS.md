...
- **usecase** — orchestration. Interfaces (`storage`, `notifierClient`, ...) are
  ...
  Use cases import no clients, storage packages, handlers, or DI. Multi-write
  flows are wrapped in `trm.Do`.
...
make lint           # golangci-lint v2.11.0, matching CI
...
- **Error mapping** — storage adapters return domain not-found errors from
  `model`; use cases translate them to operation errors; handlers translate to HTTP.
- **Outbound mapping** — use-case ports accept domain events; clients own and
  populate private wire DTOs.
